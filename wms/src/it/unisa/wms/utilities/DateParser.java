package it.unisa.wms.utilities;
...
public class DateParser {
  ...
  public static final Pattern DATEPATTERN = Pattern.compile(WBNEG + DATE+"|(\\d{1,2}[/-]{1})"); // No trailing boundary because of '#'
  ...
  private static final String[] MONTHS_IT = new String[] { "gen", "feb", "mar", "apr", "mag", "giu", "lug", "ago", "set", "ott", "nov", "dic" };
  ...
  private static final Pattern monthPatternDe = Pattern.compile(WB + "(Jan|Januar|Feb|Februar|Febr|M\u00E4r|M\u00E4rz|Apr|April|Mai|Jun|Juni|Jul|Juli|Aug|August|Sep|September|Sept" + "|Okt|Oktober|Nov|November|Dez|Dezember)" + WB);

  private static final Pattern monthPatternEs = Pattern.compile(WB + "(ene|enero|feb|febrero|febr|mar|marzo|abr|abril|may|mayo|jun|junio|julio|ago|agosto|sep|septiembre" + "|oct|octubre|nov|noviembre|dic|diciembre)" + WB);

  private static final Pattern monthPatternFr = Pattern.compile(WB + "(janv|janvier|f\u00E9vr|f\u00E9vrier|mars|avr|avril|mai|juin|juil|juillet|ao\u00FBt|sept|septembre" + "|oct|octobre|nov|novembre|d\u00E9c|d\u00E9cembre)" + WB);

  private static final Pattern monthPatternIt = Pattern.compile(WB + "(gen|gennaio|feb|febbraio|febr|mar|marzo|apr|aprile|mag|maggio|giu|giugno|lug|luglio|ago|agosto|set|settembre" + "|ott|ottobre|nov|novembre|dic|dicembre)" + WB);
  ...
  */
  ...
  public static String normalize(CharSequence s, String language) {
  ...
  public static String normalize(CharSequence s, PositionTracker posTracker) {
    return normalize(s, "ENGLISH", posTracker);
  }
  ...
  public static String normalize(CharSequence s, String language, PositionTracker posTracker) {
    Pattern monthPattern;
    String[] months = null;
    if (language.equals("ENGLISH")) {
      months = MONTHS_EN;
  ...
  public static Collection<String> getAllDates(CharSequence s) {
    return getAllDates(s, "ENGLISH");
  }

  /** Normalizes all dates in a String */
  public static Collection<String> getAllDates(CharSequence s, String language) {
  ...
  public static Collection<Triple<String, Integer, Integer>> getAllDatePositions(CharSequence s) {
    return getAllDatePositions(s, "ITALIAN");
  }

  /** Returns all the dates values and their position in the text */
  public static Collection<Triple<String, Integer, Integer>> getAllDatePositions(CharSequence s, String language) {
    Pattern monthPattern;
    String[] months = null;
    if (language.equals("ENGLISH")) {
      months = MONTHS_EN;
  ...
  public static void main(String[] argv) throws Exception {
    /*System.out.println(NumberFormatter.ISOtime(DateParser.asCalendar(DateParser.normalize(" oroeiotrwre October 24th 1998"))));
    ...
    while (true) {
	  ...
	  String testo="Nel 2008 ho divorziato e sono divenuta titolare di assegno divorzile. Nel 2009 il mio ex marito si � risposato e poi nel 2011 � deceduto. 9 marzo 1940 Ho diritto 2010 all'attribuzione di una percentuale della pensione di reversibilit�?";
	  
	  System.out.println(removeDate(testo));
  }
  
  public static String removeDate(String fromText) {
		Collection<Triple<String, Integer, Integer>> c=DateParser.getAllDatePositions(fromText);
		Iterator<Triple<String, Integer, Integer>> it=c.iterator();
		String result="";
		int begin=0;
		int end=0;
		while(it.hasNext()){
			Triple<String,Integer,Integer> tripla=it.next();
			try{
				end=tripla.second();
				result+=fromText.substring(begin, end);
				begin=end+tripla.third();
			} catch (StringIndexOutOfBoundsException e) {
			//	System.out.println("IGNORA data:"+tripla.first+" Pos:"+tripla.second+" Length:"+tripla.third);
			}
		}
		result+=fromText.substring(begin);
		result=DateParser.YEARPATTERN.matcher(result).replaceAll("");
		result=DateParser.DATEPATTERN.matcher(result).replaceAll("");
		return result;
	}
